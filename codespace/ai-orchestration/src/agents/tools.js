...
export const listFiles = tool(
    async ({ }) => {
        console.log("Listing files in project directory...\n");
        
        const response = await axios.get(`http://019e35c1-42eb-726a-b96f-3250ba2aebe4.agent.localhost/list-files`)

        // writer("Files listed successfully." + "Files: " + response.data.files.join(",") + "\n");

        console.log("Files listed successfully." + "Files: " + response.data.files.join(",") + "\n");
...
export const readFiles = tool(
    async ({ files = [] }) => {
        ...
        const response = await axios.get(`http://019e35c1-42eb-726a-b96f-3250ba2aebe4.agent.localhost/read-files?files=` + files.join(","))
...
export const updateFiles = tool(
    async ({ files }) => {
        ...
        // const writer = config.writer;
        console.log("Updating files..." + files.map(f => f.file).join(",") + "\n");
        
        // writer("Updating files..." + files.map(f => f.file).join(",") + "\n");


        const response = await axios.patch(`http://019e35c1-42eb-726a-b96f-3250ba2aebe4.agent.localhost/update-files`, {
            updates: files
        })
